...
foreach ($courses as $key => $value) {
    ...
    <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12 one_course">
        <a href="<?php echo base_url() . $value['slug'] . '-2' . $value['id']; ?>.html?link=321" title="<?php echo $value['name']; ?>">
            <div class="img-course"> 
                <img src="<?php echo 'https://lakita.vn/' . $value['image']; ?>" alt="<?php echo $value['name']; ?>, học excel cơ bản, hoc excel co ban, excel kế toán, excel ke toan, làm chủ hóa đơn chứng từ, lam chu hoa don chung tu, lập báo cáo tài chính, lap bao cao tai chinh" 
                    ...
                    <figure class="bginfo">
                        ...
                        <p>Giá: <strong><?php
                                if ($value['time_start_sale'] < time() && $value['time_end_sale'] > time() & $value['time_start_sale'] != 0 & $value['time_start_sale'] != 0) {
        ...
        <?php if ($value['time_start_sale'] < time() && $value['time_end_sale'] > time() & $value['time_start_sale'] != 0 & $value['time_start_sale'] != 0) { ?>
            <p class="price"> <?php echo number_format(str_replace('.', '', $value['price_root2']), 0, ',', '.') . " đ"; ?></p>
            <p class="price2"> <?php echo number_format(str_replace('.', '', $value['price_sale']), 0, ',', '.') . " đ"; ?></p>
        <?php } else { ?>
